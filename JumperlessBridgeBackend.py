...
def get_available_ports(): #this should return the list of ports, and send the port names to the gui
...
def find_main_port(jumperless_ports, force_quit_python=False):
    """Find the main Jumperless port using ENQ (0x05) query in numerical order
    
    The main port is identified by:
    1. Testing ports in numerical order (port1, port2, port3, etc.)
    2. Sending ENQ (0x05) to query CDC port functions
    3. Looking for response with CDC port list (e.g. "CDC0: Jumperless Main")
    4. Main port typically responds with multiple CDC entries or "Main" identifier
    """
    
    port_errors = []
    
    # Sort ports in numerical order by extracting the port number
    def extract_port_number(port_tuple):
        port_name = port_tuple[0]  # First element is port name
        # Extract number from port name like "/dev/cu.usbmodemJLV5port1" -> 1
        try:
            if 'port' in port_name:
                return int(port_name.split('port')[-1])
            else:
                return 999  # Put non-numbered ports at the end
        except:
            return 999
    
    # Sort ports by number
    sorted_ports = sorted(jumperless_ports, key=extract_port_number)
    
    safe_print("Testing ports in numerical order using ENQ (0x05) query:", Fore.CYAN)
    for i, (port_name, desc, hwid, interface, additional_attrs) in enumerate(sorted_ports):
        port_num = extract_port_number((port_name, desc, hwid, interface, additional_attrs))
        safe_print(f"  {i+1}. {port_name} (port {port_num}) - {desc}", Fore.BLUE)
    
    for port_name, desc, hwid, interface, additional_attrs in sorted_ports:
        try:
            safe_print(f"Testing {port_name} with ENQ query...", Fore.CYAN)
            
            # Try to open the port
            try:
                test_port = serial.Serial(port_name, 115200, timeout=0.5)
            except serial.SerialException as e:
                error_msg = f"Port {port_name} unavailable: {str(e)}"
                port_errors.append(error_msg)
                safe_print(error_msg, Fore.RED)
                continue
            ...
            try:
                ...
                # Send quit commands if requested to clear any active sessions
                if force_quit_python:
                    safe_print(f"Clearing any active sessions on {port_name}...", Fore.YELLOW)
                    
                    # Send exit commands to ensure we're at the base prompt
                    exit_commands = [
                    ...
                    # Clear any response and wait
                    test_port.reset_input_buffer()
                    time.sleep(0.3)
                
                # Send ENQ (0x05) to query CDC port functions
                safe_print(f"Sending ENQ (0x05) to {port_name}...", Fore.CYAN)
                test_port.reset_input_buffer()  # Clear before query
                test_port.write(b"\x05\n")
                test_port.flush()
                ...
                if test_port.in_waiting > 0:
                    ...
                    if response_buffer:
                        try:
                            ...
                            safe_print(f"ENQ response from {port_name}:", Fore.BLUE)
                            
                            # Show only CDC lines for clarity
                            cdc_lines = [line.strip() for line in response_str.split('\n') 
                                       if line.strip().startswith('CDC')]
                            
                            if cdc_lines:
                                for line in cdc_lines:
                                    safe_print(f"  {line}", Fore.GREEN)
                                
                                # The main port should respond with multiple CDC entries
                                # typically "CDC0: Jumperless Main" plus others
                                main_port_indicators = [
                                    "Jumperless Main",
                                    "JL Main", 
                                    "Main",
                                    "Communication"
                                ...
                                has_main_indicator = any(indicator in response_str 
                                                       for indicator in main_port_indicators)
                                has_multiple_cdc = len(cdc_lines) >= 2  # Main port lists multiple CDC ports
                                
                                if has_main_indicator or has_multiple_cdc:
                                    safe_print(f"✅ Found main port: {port_name}", Fore.GREEN)
                                    safe_print(f"   Reason: {'Main port indicator found' if has_main_indicator else 'Multiple CDC ports listed'}", Fore.GREEN)
                                    test_port.close()
                                    return port_name
                                else:
                                    safe_print(f"Port {port_name} responded but appears to be a secondary CDC port", Fore.YELLOW)
                            else:
                                # No CDC lines, might be in menu mode or other state
                                if "Menu" in response_str or "'help'" in response_str:
                                    safe_print(f"Port {port_name} appears to be in menu mode", Fore.YELLOW)
                                else:
                                    safe_print(f"Port {port_name} ENQ response (no CDC info):", Fore.YELLOW)
                                    # Show first 200 chars of response for debugging
                                    safe_print(f"  {repr(response_str[:200])}", Fore.YELLOW)
                        
                        except Exception as e:
                            error_msg = f"Error decoding ENQ response from {port_name}: {e}"
                            port_errors.append(error_msg)
                            safe_print(error_msg, Fore.YELLOW)
                    else:
                        safe_print(f"Port {port_name} did not respond to ENQ query", Fore.YELLOW)
                else:
                    safe_print(f"No response from {port_name} to ENQ query", Fore.YELLOW)
                        
            finally:
                test_port.close()
            
        except Exception as e:
            error_msg = f"Error testing {port_name}: {e}"
            port_errors.append(error_msg)
            safe_print(error_msg, Fore.RED)
    
    # If we get here, no main port was found
    safe_print("❌ Could not find main port using ENQ queries", Fore.RED)
    if port_errors:
        safe_print("Error summary:", Fore.RED)
        for error in port_errors:
            safe_print(f"  • {error}", Fore.YELLOW)
    
    safe_print("💡 Troubleshooting:", Fore.CYAN)
    safe_print("  • The main port should respond to ENQ with CDC port list", Fore.BLUE)
    safe_print("  • Make sure no other programs are using the ports", Fore.BLUE)  
    safe_print("  • Try disconnecting and reconnecting the device", Fore.BLUE)
    safe_print("  • Check if the device is in menu or interactive mode", Fore.BLUE)
...
def get_all_port_functions(main_port_name, all_jumperless_ports): #this should return the port functions, and send the port function names to the gui
...
def open_serial(): #this should return the port name, nand send the port name to the gui
...
def update_jumperless_firmware(force=False): #this should be exposed to the gui now
...
def bridge_menu(): #these menus should be handled by the gui now
    """Main bridge menu"""
...
def serial_term_out():
    ...
    while True:#this should actually just send stuff when it's coming from the app, like wokwi updates, not user input now
...
# ============================================================================
# BACKEND INTERFACE FOR GUI
# ============================================================================

class JumperlessBridgeBackend:
    """Backend interface for GUI integration"""
    
    def __init__(self, gui_mode=False):
        """Initialize backend in standalone or GUI mode"""
        self.gui_mode = gui_mode
        self.gui_callback = None
        self.backend_threads = []
        self.is_running = False
        self.manual_port_selection_result = None
        self.waiting_for_port_selection = False
        
        # Override safe_print for GUI mode
        if self.gui_mode:
            self._setup_gui_output_redirection()
        
    def set_gui_callback(self, callback):
        """Set callback function for sending updates to GUI"""
        self.gui_callback = callback
        
        # Now that we have the callback, redirect output if in GUI mode
        if self.gui_mode:
            self._redirect_safe_print()
        
    def _setup_gui_output_redirection(self):
        """Set up output redirection for GUI mode"""
        # Store original safe_print function
        global safe_print
        self._original_safe_print = safe_print
        
    def _redirect_safe_print(self):
        """Redirect safe_print to GUI output"""
        global safe_print, input
        
        # Store reference to self for the inner functions
        backend_instance = self
        
        def gui_safe_print(text, color=None, end='\n', file=None):
            """Safe print function that sends output to GUI"""
            if backend_instance.gui_callback:
                # Convert colorama colors to GUI color names
                color_map = {
                    Fore.RED: 'red',
                    Fore.GREEN: 'green', 
                    Fore.BLUE: 'blue',
                ...
                gui_color = color_map.get(color, 'white')
                backend_instance.gui_callback('backend_output', {
                    'message': str(text), 
                    'color': gui_color,
                    'end': end
                })
            else:
                # Fallback to original safe_print
                backend_instance._original_safe_print(text, color, end)
        
        def gui_input(prompt=""):
            """Input function that works with GUI"""
            if "Enter port number" in prompt and backend_instance.gui_callback:
                # This is a port selection prompt - handle via GUI
                backend_instance.waiting_for_port_selection = True
                backend_instance.manual_port_selection_result = None
                
                # Send port selection request to GUI
                backend_instance.gui_callback('port_selection_required', {
                    'prompt': prompt,
                    'message': 'Please select a port from the dropdown and click Connect, or use the Rescan button to refresh the list.'
                })
                
                # Wait for GUI to provide selection (with timeout)
                import time
                timeout = 30  # 30 second timeout
                start_time = time.time()
                
                while backend_instance.waiting_for_port_selection and (time.time() - start_time) < timeout:
                    time.sleep(0.1)
                
                if backend_instance.manual_port_selection_result is not None:
                    return backend_instance.manual_port_selection_result
                else:
                    # Timeout or no selection - use GUI controls instead
                    return "gui_controlled"
            else:
                # For non-port-selection prompts, use the original input
                return backend_instance._original_input(prompt)
        
        # Replace the global functions in this module
        safe_print = gui_safe_print
        
        # Also update the global namespace 
        globals()['safe_print'] = gui_safe_print
        
        # Store original input and replace it
        self._original_input = input
        input = gui_input
        globals()['input'] = gui_input
        
    def _restore_safe_print(self):
        """Restore original safe_print and input functions"""
        global safe_print, input
        if hasattr(self, '_original_safe_print'):
            safe_print = self._original_safe_print
            globals()['safe_print'] = self._original_safe_print
        if hasattr(self, '_original_input'):
            input = self._original_input
            globals()['input'] = self._original_input
    
    def set_port_selection_result(self, result):
        """Set the result of manual port selection from GUI"""
        self.manual_port_selection_result = result
        self.waiting_for_port_selection = False
        
    def _notify_gui(self, event_type, data=None):
        """Send notification to GUI if callback is set"""
        if self.gui_callback:
            try:
                self.gui_callback(event_type, data)
            except Exception as e:
                # Use original safe_print to avoid recursion
                if hasattr(self, '_original_safe_print'):
                    self._original_safe_print(f"GUI callback error: {e}", Fore.RED)
                else:
                    print(f"GUI callback error: {e}")
    
    def initialize(self):
        """Initialize backend systems - in GUI mode, find port then disconnect"""
        global active_threads
        ...
        # Open serial connection to detect device and get info
        serial_result = open_serial()
        if not serial_result:
            self._notify_gui('connection_failed', {'error': 'Could not establish serial connection'})
            return False
            
        # Check firmware and get device info
        check_if_fw_is_old()
        
        # Store device info for GUI
        device_info = {
            'port': portName,
            'arduino_port': arduinoPort,
            'firmware_version': currentString,
            'is_v5': jumperlessV5
        }
        
        # In GUI mode, disconnect from serial after detection - GUI will handle serial I/O
        if self.gui_mode:
            self._disconnect_backend_serial()
            self._notify_gui('backend_output', {
                'message': "🔌 Backend released serial port to GUI - operating in Wokwi-only mode", 
                'color': 'cyan'
            })
        ...
        self._notify_gui('backend_initialized', device_info)
        
        return True
    
    def _disconnect_backend_serial(self):
        """Disconnect backend from serial port completely in GUI mode"""
        ...
        try:
            if ser and ser.is_open:
                ser.close()
                self._notify_gui('backend_output', {
                    'message': "✅ Backend serial connection closed", 
                    'color': 'green'
                })
        except Exception as e:
            self._notify_gui('backend_output', {
                'message': f"⚠️ Error closing backend serial: {e}", 
                'color': 'yellow'
            })
        
        # Clear backend serial references - GUI will manage its own connection
        ser = None
        serialconnected = 0
        
    def _start_backend_threads(self):
        """Start essential backend threads - minimal in GUI mode"""
        global active_threads
        
        if self.gui_mode:
            # In GUI mode, only start Wokwi processing thread - no serial monitoring
            self._notify_gui('backend_output', {
                'message': "🔄 Starting Wokwi processing thread (serial handled by GUI)", 
                'color': 'blue'
            })
        else:
            # In CLI mode, start full monitoring
            ...
            self.backend_threads.append(port_monitor)
            
            # Serial input thread
            serial_in = threading.Thread(target=serial_term_in, daemon=True)
            serial_in.start()
            active_threads.append(serial_in)
            self.backend_threads.append(serial_in)
            
            # Serial output thread
            serial_out = threading.Thread(target=serial_term_out, daemon=True)
            serial_out.start()
            active_threads.append(serial_out)
            self.backend_threads.append(serial_out)
        
    def _temporary_serial_connect(self):
        """Temporarily connect to serial for Wokwi operations, then disconnect"""
        global ser, serialconnected, portName
        
        if not portName:
            return False
            
        try:
            # Quick temporary connection
            ser = serial.Serial(port=portName, baudrate=115200, timeout=1)
            serialconnected = 1
            return True
        except Exception as e:
            self._notify_gui('backend_output', {
                'message': f"⚠️ Temporary serial connection failed: {e}", 
                'color': 'yellow'
            })
            return False
    
    def _temporary_serial_disconnect(self):
        """Disconnect from serial after Wokwi operation"""
        global ser, serialconnected
        ...
        with serial_lock:
            if serialconnected and ser and ser.is_open:
                try:
                    ...
                    self._notify_gui('serial_error', {'error': str(e)})
                    return False
            else:
                self._notify_gui('serial_error', {'error': 'Not connected'})
                return False
    ...
    def scan_ports(self):
        """Scan for available ports and return organized port info"""
        try:
            # Notify GUI that we're starting port detection
            self._notify_gui('port_scan_started', {})
            
            ports = get_available_ports()
            if not ports:
                return None
                
            # Find Jumperless ports
            jumperless_ports = []
            for port, desc, hwid, interface, additional_attrs in ports:
                vid, pid = parse_hardware_id(hwid, desc)
                if is_jumperless_device(desc, pid, interface):
            ...
            # Notify GUI to temporarily release any WebSocket serial connections
            # so we can do port detection
            self._notify_gui('release_serial_for_detection', {})
            
            # Give GUI time to release connections
            import time
            time.sleep(0.5)
            ...
            if main_port_name:
                # Get port functions and organize
                port_functions = get_all_port_functions(main_port_name, jumperless_ports)
                organized_ports = organize_jumperless_ports(main_port_name, jumperless_ports, port_functions)
                arduino_port = choose_arduino_port(organized_ports, main_port_name)
                
                # Notify GUI that detection is complete
                self._notify_gui('port_scan_completed', {
                    'main_port': main_port_name,
                    'arduino_port': arduino_port,
                    'organized_ports': organized_ports
                })
                ...
                return {
                    ...
                    'arduino_port': arduino_port,
                    'organized_ports': organized_ports,
                    'all_ports': jumperless_ports
                }
            else:
                # If we can't find main port, return basic port info for manual selection
                self._notify_gui('manual_port_selection_needed', {
                    'jumperless_ports': jumperless_ports,
                    'message': 'Could not automatically detect main port. Please select manually.'
                })
                
                # Create basic organized ports dict for manual selection
                organized_ports = {}
                for i, (port, desc, hwid, interface, additional_attrs) in enumerate(jumperless_ports):
                    organized_ports[port] = desc
            ...
            self._notify_gui('scan_error', {'error': str(e)})
            return None
    
    def connect_to_port(self, port_name):
        """Connect to a specific port"""
        global portName, ser, serialconnected
        ...
        try:
            ...
            self._notify_gui('connection_established', {'port': port_name})
            return True
            
        except Exception as e:
            self._notify_gui('connection_failed', {'error': str(e), 'port': port_name})
            return False
    ...
    def update_firmware(self, force=False):
        """Update Jumperless firmware"""
        try:
            result = update_jumperless_firmware(force=force)
            self._notify_gui('firmware_update_complete', {'success': result})
            return result
        except Exception as e:
            self._notify_gui('firmware_update_failed', {'error': str(e)})
            return False
    
    def flash_arduino(self, sketch_content, libraries_content="", slot_number=None):
        """Flash Arduino sketch"""
        try:
            thread = flash_arduino_sketch_threaded(sketch_content, libraries_content, slot_number)
            return thread
        except Exception as e:
            self._notify_gui('flash_failed', {'error': str(e)})
            return None
    
    def get_wokwi_slots(self):
        """Get current Wokwi slot assignments"""
        try:
            return count_assigned_slots()
        except Exception as e:
            self._notify_gui('slots_error', {'error': str(e)})
            return 0
    
    def assign_wokwi_slots(self):
        """Open Wokwi slot assignment interface"""
        try:
            return assign_wokwi_slots()
        except Exception as e:
            self._notify_gui('slots_error', {'error': str(e)})
    ...
    def toggle_wokwi_updates(self):
        """Toggle Wokwi update functionality"""
        global noWokwiStuff
        noWokwiStuff = not noWokwiStuff
        self._notify_gui('wokwi_toggled', {'enabled': not noWokwiStuff})
        return not noWokwiStuff
    
    def toggle_arduino_flashing(self):
        """Toggle Arduino flashing functionality"""
        global disableArduinoFlashing, noArduinocli
        if noArduinocli:
            self._notify_gui('arduino_cli_unavailable', {})
            return False
        
        disableArduinoFlashing = not disableArduinoFlashing
        self._notify_gui('arduino_flashing_toggled', {'enabled': not disableArduinoFlashing})
        return not disableArduinoFlashing
    
    def toggle_debug_mode(self):
        """Toggle debug output"""
        global debugWokwi
        debugWokwi = not debugWokwi
        self._notify_gui('debug_toggled', {'enabled': debugWokwi})
        return debugWokwi
    
    def set_wokwi_update_rate(self, rate):
        """Set Wokwi update rate"""
        global wokwiUpdateRate
        if 0.5 <= rate <= 50.0:
            wokwiUpdateRate = rate - 0.4
            self._notify_gui('update_rate_changed', {'rate': rate})
            return True
        return False
    ...
    def get_status(self):
        ...
        try:
            # Check Arduino port status
            arduino_port_status = False
            if arduinoPort:
                try:
                    test_port = serial.Serial(arduinoPort, 115200, timeout=0.1)
                    test_port.close()
                    arduino_port_status = True
                except:
                    arduino_port_status = False
            ...
            return {
                'main_port': portName,
                'arduino_port': arduinoPort,
                'main_connected': serialconnected,
                'arduino_connectable': arduino_port_status,
                'firmware_version': currentString,
                'is_v5': jumperlessV5,
                'arduino_cli_available': not noArduinocli,
                'arduino_flashing_enabled': not disableArduinoFlashing and not noArduinocli,
                'wokwi_enabled': not noWokwiStuff,
                'debug_enabled': debugWokwi,
                'interactive_mode': interactive_mode,
                'assigned_slots': numAssignedSlots,
                'update_rate': wokwiUpdateRate + 0.4
            }
        except Exception as e:
            return {'error': str(e)}
    
    def start_wokwi_loop(self):
        """Start Wokwi monitoring loop in a separate thread"""
        if self.gui_mode and not noWokwiStuff:
            wokwi_thread = threading.Thread(target=self._wokwi_loop, daemon=True)
            wokwi_thread.start()
            self.backend_threads.append(wokwi_thread)
            return wokwi_thread
        return None
    
    def _wokwi_loop(self):
        """Optimized Wokwi monitoring loop - only connects for updates, then disconnects"""
        global forceWokwiUpdate, currentSlotUpdate, lastDiagram, blankDiagrams, justreconnected, wokwiUpdateRate
        
        self._notify_gui('backend_output', {
            'message': "🔄 Wokwi loop started - waiting for update requests", 
            'color': 'blue'
        })
        
        while self.is_running and not noWokwiStuff:
            # Wait longer when menu/update in progress or when idle
            if menuEntered or updateInProgress:
                time.sleep(1.0)  # Long sleep when busy
                continue
            
            # Check if we need to do any Wokwi work
            need_wokwi_update = False
            
            with wokwi_update_lock:
                if forceWokwiUpdate or justreconnected:
                    need_wokwi_update = True
                    ...
                    if justreconnected:
                        ...
                        currentSlotUpdate = 0
                        justreconnected = 0
                        self._notify_gui('reconnected', {})
            
            if need_wokwi_update:
                # Only now temporarily connect for Wokwi operations
                self._notify_gui('backend_output', {
                    'message': f"🔌 Temporarily connecting for Wokwi update (slot {currentSlotUpdate})", 
                    'color': 'cyan'
                })
                
                if self._temporary_serial_connect():
                    try:
                        # Process Wokwi slots here (existing logic would go here)
                        # This is where you'd put the actual Wokwi processing
                        time.sleep(0.1)  # Simulate processing time
                        
                        self._notify_gui('backend_output', {
                            'message': f"✅ Wokwi update completed for slot {currentSlotUpdate}", 
                            'color': 'green'
                        })
                        
                    except Exception as e:
                        self._notify_gui('wokwi_error', {'error': str(e), 'slot': currentSlotUpdate})
                    finally:
                        # Always disconnect after Wokwi operation
                        self._temporary_serial_disconnect()
                        self._notify_gui('backend_output', {
                            'message': "🔌 Disconnected after Wokwi update", 
                            'color': 'blue'
                        })
            ...
            # Wait for the configured interval, but at least 1 second to reduce CPU usage
            wait_time = max(wokwiUpdateRate, 1.0)  # Minimum 1 second between checks
    ...
    def shutdown(self):
        ...
        # Restore original safe_print function
        if self.gui_mode:
            self._restore_safe_print()
        ...
        self._notify_gui('backend_shutdown', {})

# Global backend instance for GUI use
_backend_instance = None

def get_backend_instance(gui_mode=True):
    """Get or create the global backend instance"""
    global _backend_instance
    if _backend_instance is None:
        _backend_instance = JumperlessBridgeBackend(gui_mode=gui_mode)
